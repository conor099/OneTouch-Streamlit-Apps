...
def connect_to_sql_server(server_type):
    """
    ...
    if server_type == "dev":
        ...
        raise ValueError("server_type must equal 'dev' or 'prod'.")
...
def connect_to_sql_alchemy_server(server_type):
    """
    :param server_type: Takes values dev (development, localhost) or prod (production server).
    :param   server:    Name of SQL server you want to connect to.
    :param   database:  Name of database you want to connect to.
    ...
    if server_type == "dev":
        ...
        engine = alc.create_engine(conn_string, echo=False, fast_executemany=True)
...
def select_sql_table(table_schema, table_name, connection, columns="*"):
    """
    ...
    :param   columns:      List of column names you want to select from table. Default is all columns.
    :return: table_df:     Pandas dataframe of table from SQL.
    """
    # Convert list to selection of columns in SQL.
    if columns != "*":
        columns = ', '.join([f"{col}" for col in columns])

    # Determine whether the object is a table or view
    inspector = alc.inspect(connection)
    object_type = None
    for t in inspector.get_view_names(schema=table_schema):
        if t.lower() == table_name.lower():
            object_type = 'view'
            break
    if object_type is None:
        for t in inspector.get_table_names(schema=table_schema):
            if t.lower() == table_name.lower():
                object_type = 'table'
                break

    if object_type is None:
        raise ValueError(f"Object '{table_schema}.{table_name}' not found in the database.")

    # Build SQL query
    if object_type == "table":
        input_query = f"""SELECT {columns} 
...
def execute_stored_procedure(table_schema, table_name, connection, print_updated_rows=0, full_load=0):
    """
    :param table_schema:       Schema for table/view in SQL.
    :param table_name:         Name of table/view you want to update.
    :param connection:         Connection to SQL server.
    :param print_updated_rows: Equals 1 or 0. 1 if we want to print how many rows the SP updated in the table.
    :param full_load:          Equals 1 or 0. 1 if we want to truncate the table before executing the SP to update.
    """
    # Print initial start message.
    print(f"Now updating [{table_schema}].[{table_name}]")

    # Initialise start time to see how long it takes to find all games.
    start_time = time.time()

    # Truncate table if full_load = 1.
    if full_load == 1:
        with connection.connect() as conn:
            conn.execute(alc.text(f"""TRUNCATE TABLE [{table_schema}].[{table_name}]"""))
            conn.commit()

    # Find how many rows are in the table before updating.
    if print_updated_rows == 1:
        rows_before = int(pd.read_sql_query(sql=f"""SELECT COUNT(*) FROM {table_schema}.{table_name}""", con=connection).iloc[0, 0])
        print(f"Rows before update = {rows_before}")

    # Execute the SP.
    with connection.connect() as conn:
        conn.execute(alc.text(f"""EXEC [{table_schema}].[Update_{table_name}]"""))
        conn.commit()

    # Find how many rows are in the table after updating.
    if print_updated_rows == 1:
        rows_after = int(pd.read_sql_query(sql=f"""SELECT COUNT(*) FROM [{table_schema}].[{table_name}]""", con=connection).iloc[0, 0])
        print(f"Rows after update = {rows_after}")
        print(f"Number of new rows = {rows_after - rows_before}")

    # Finish time.
    end_time = time.time()

    print(f"Executed stored procedure [{table_schema}].[Update_{table_name}] in {(end_time - start_time) / 60} minutes.")


#%% Transfer a table from one SQL server to another.

def transfer_table_from_one_server_to_another(source_server_type, target_server_type, table_schema, table_name,
                                              batch_size=1000):
    """
    :param source_server_type: Takes values dev (development, localhost) or prod (production server).
    :param target_server_type: Takes values dev (development, localhost) or prod (production server).
    :param table_schema:       Schema of source/target table.
    :param table_name:         Name of source/target table.
    :param batch_size:         Number of rows to insert per batch.
    :return:                   Table copied to target server/database.
    """
    # Start time.
    start_time = time.time()

    # Connect to source and target databases.
    source_connection = connect_to_sql_alchemy_server(server_type=source_server_type)
    target_connection = connect_to_sql_alchemy_server(server_type=target_server_type)

    # Select all rows from source table.
    source_query = f"""SELECT * FROM [{table_schema}].[{table_name}]"""
    source_table = pd.read_sql_query(sql=source_query, con=source_connection)
    print(f"Loaded table [{table_schema}].[{table_name}].")

    # Extract all datatypes from the source table.
    metadata = alc.MetaData()
    meta_table = alc.Table(table_name, metadata, autoload_with=source_connection, schema=table_schema)
    data_types = {col.name: col.type for col in meta_table.columns}
    print("Extracted datatypes of source table.")

    with target_connection.begin() as target_conn:
        # Truncate table on target server.
        target_conn.execute(alc.text(f"""TRUNCATE TABLE [{table_schema}].[{table_name}]"""))
        print("Truncated table on target database.")

        # Turn on IDENTITY_INSERT.
        target_conn.execute(alc.text(f"SET IDENTITY_INSERT [{table_schema}].[{table_name}] ON"))

        # Insert data into SQL database table.
        source_table.to_sql(name=table_name, con=target_conn, schema=table_schema, dtype=data_types,
                            if_exists="append", index=False, chunksize=batch_size, method=None)

        # End time.
        end_time = time.time()

        print(f"Inserted [{table_schema}].[{table_name}] on target database in {(end_time - start_time)/60} minutes.")

        # Turn off IDENTITY_INSERT.
        target_conn.execute(alc.text(f"SET IDENTITY_INSERT [{table_schema}].[{table_name}] OFF"))
...
def SB_get_unique_games(comp_season_ids):
    ...
    # Calculate time it takes to add one game's data to table.
    end_time = time.time()
    print(f"Time to find all matches = {(end_time - start_time) / 60} minutes.")

    return set(sb_game_tuples), set(sb_game_ids)
...
def drop_useless_columns(df):
    ...
    for column_name in df.columns:
        ...
        # If column name is hashbyte or generic name id, these columns should be dropped.
        elif column_name == "hashbyte" or column_name == "id":
            drop_columns.append(column_name)
